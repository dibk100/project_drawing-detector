...
🧩 OCR Library
   - Tesseract OCR(86개) : 실선을 텍스트로 오인식, 한글을 문자단위로 인식
   - EasyOCR(36개) : 숫자 인식률 낮음, 심볼의 텍스트 인식 불가, 비교적 한글과 영어 성능 좋음
   - PaddleOCR(40개) : 한국어+영어 혼합 인식에서 가장 안정적. 심복의 텍스트는 부분적으로 탐지되지만 명확한 추출 불가

🧩 VLM
   - Gemini 2.5 Flash(34개) : 한국어, 영어 성능이 좋지만, 텍스트 경계 정보(좌표)가 명확하지 않아서 OCR 대비 인식 안정성 낮음. _이것도 프롬프트 설정 가능하면 좌표도 뽑을 수 있지 않을까?_
